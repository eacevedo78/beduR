...
pbinom(q=3, size=10,prob=0.3,lower.tail=TRUE) #P(X <= x)
#Si lower.tail fuera falso calcula la probabilidad mayor al 3 en nuestro caso.



"¿Cuál es la probabilidad de que más de 5 clientes realicen una compra?"

# dbinom es para la masa de probabilidad
dbinom(x = 6, size = 10, prob = 0.3) +
...
"Para una distribución binomial tenemos que:
    - E[X] = size*p  es el valor esperado o media
    - SD[X] = sqrt(size*p*(1-p))  la desviacion estandar
    
#la diferencia entre bernoulli y binomial es que
#de bernouli es un experimento, y la binomial se ejecuta mas veces
Esto podemos comprobarlo con las funciones descriptivas que hemos visto:"

mean(binom) #Es el valor esperado, es decir se esperan 3 clientes.
sd(binom) 
...
{
  #Leptocurtica  puntiaguda, desviacion estandar  menor.
  curve(dnorm(x, mean = 150, sd = 5), from = 120, to = 180, 
        col='blue', main = "Densidad Normal:\nDiferente desviación estándar",
        ylab = "f(x)", xlab = "X")
  #Mesocurtica grafica intermedia, desviacion estandar a la mitad aproximadamente
  curve(dnorm(x, mean = 150, sd = 10), from = 120, to = 180, 
        col='red', add = TRUE)
  #Platocurtica mayor desviacion estandar, mas forma de plato
...
#pnorm es la probabilidad ACUMULADA
...
# z me indica cuantas "desviaciones estandar" estamos alejados de la media
